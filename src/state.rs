...
impl State {
	pub fn create_entry(&self, name: String) -> Arc<EntryState> {
		let mut entries = self.entries.lock().expect("i literally don't know what a poisoned thread is");
		let entry = entries.entry(name).or_insert_with(|| Arc::new(EntryState::default()));
		Arc::clone(entry)
	}
	...
	{
		let entries = self.entries.lock().expect("i literally don't know what a poisoned thread is");
		entries.get(span.name()).map(|entry| Arc::clone(entry))
	}
}